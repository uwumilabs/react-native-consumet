...
import { AnimeParser, StreamingServers, MediaFormat, SubOrSub, MediaStatus, } from '../../models';
    ...
    constructor(customBaseURL) {
        ...
        this.search = async (query, page = 1) => {
            ...
            res.results = data.results.map((item) => ({
                ...
                image: `${this.baseUrl}${item.image}` || `${this.baseUrl}${item.thumbnail}` || `${this.baseUrl}${item.webp}`,
        ...
        this.fetchAnimeInfo = async (id) => {
            ...
            try {
                ...
                let totalSubEpisodes = parseInt($('div#anime-cover-sub-content > div.nav-container > ul#episode-list > li.nav-item')
                    ...
                    .split('-')[1]
                    .trim());
                let totalDubEpisodes = parseInt($('div#anime-cover-dub-content > div.nav-container > ul#episode-list > li.nav-item')
                    .last()
                    .text()
                    .split('-')[1]
                    ?.trim() ?? '0');
         ...
         */
        this.fetchEpisodeSources = async (episodeId, server = StreamingServers.Luffy, subOrDub = SubOrSub.SUB) => {
         ...
         */
        this.fetchEpisodeServers = async (episodeId, subOrDub = SubOrSub.SUB) => {
