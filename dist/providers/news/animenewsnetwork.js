...
async function scrapNewsInfo(url) {
    ...
    const thumbnailHash = getHashFromImage(thumbnailSlug ? `https://animenewsnetwork.com${thumbnailSlug}` : 'https://i.imgur.com/KkkVr1g.png');
    return {
        id: url.split('news/')[1],
        title,
        uploadedAt: time,
...
class AnimeNewsNetwork extends NewsParser {
    constructor() {
         ...
         */
        this.fetchNewsFeeds = async (topic) => await axios
            .get(`${this.baseUrl}/news${topic && Object.values(Topics).includes(topic) ? `/?topic=${topic}` : ''}`)
            .then(({ data }) => {
            const $ = load(data);
            const feeds = [];
