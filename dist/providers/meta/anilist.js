...
class Anilist extends AnimeParser {
    ...
    constructor(provider, proxyConfig, adapter, customBaseURL) {
         ...
         */
        this.fetchAnimeInfo = async (id, fetchFiller = false) => {
            const animeInfo = {
                id: id,
                title: '',
            };
        ...
        this.fetchRecentEpisodes = async (provider = 'gogoanime', page = 1, perPage = 25) => {
            try {
                ...
                const results = data?.map((item) => {
                    return {
                        id: item.id.toString(),
                        malId: item.mappings.find((item) => item.providerType === 'META' && item.providerId === 'mal')?.id,
                        title: {
                            romaji: item.title?.romaji,
                            english: item.title?.english,
                            native: item.title?.native,
                        ...
                        episodeId: `${provider === 'gogoanime'
                            ? item.episodes.data
                                ...
                                ?.episodes.pop()?.id
                            : item.episodes.data.find((source) => source.providerId.toLowerCase() === 'zoro')?.episodes.pop()?.id}`,
        ...
        this.fetchDefaultEpisodeList = async (Media) => {
            let episodes = [];
         ...
         */
        this.fetchEpisodesListById = async (id, fetchFiller = false) => {
            const options = {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    ...
                    function mergeEpisodes(normalizedEpisodes, providerEpisodes) {
                        ...
                        return providerEpisodes.map((providerEp, index) => {
                            ...
                            if (!normalizedEp && providerEp.title) {
                                normalizedEp = normalizedEpisodes.find((normEp) => matchIfNotUsed(normEp) && normEp.title?.includes(providerEp.title));
                            }
