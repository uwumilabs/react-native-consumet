...
test('fetchMangaInfo: returns filled manga info when given a mangaId.', async () => {
  const data = await mangaReader.fetchMangaInfo(
    'one-piece-colored-edition-55493'
  );
  expect(data).not.toEqual({});
});

test('fetchChapterPages: returns filled page data when given a chapterId.', async () => {
  const data = await mangaReader.fetchChapterPages(
    'one-piece-colored-edition-55493/en/chapter-1004'
  );
  expect(data).not.toEqual([]);
});
