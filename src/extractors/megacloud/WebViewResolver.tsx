...
interface WebViewResolverProps {
  xrax?: string;
  onSourcesExtracted: (sources: ISource | null) => void;
}

const WebViewResolver = ({
  xrax,
  onSourcesExtracted,
}: WebViewResolverProps) => {
  ...
  (function() {
    function sendLog(data) {
      window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'log', data }));
    }

    function sendSources(data) {
      window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'sources', data }));
    }
    ...
    try {
      if (document.readyState === "complete") {
        sendLog("Document already loaded. Running script...");
        runExtraction();
      } else {
        document.addEventListener("DOMContentLoaded", function() {
          sendLog("DOMContentLoaded event fired. Running script...");
          runExtraction();
        });
      }
      ...
      async function runExtraction() {
        sendLog("Running extraction script...");
        const result = await getSources("${xrax}");
        sendLog("Add function result: " + result);
        sendSources(result);
      }
    } catch (error) {
...
function z(a) {
 ...
 window.getSources=async function(xrax) {
 window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'log', data: "getSources function initialized" }));
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (message.type === 'log') {
        ...
        console.log('WebView sources:', message.data);
        setSources(message.data);
        onSourcesExtracted?.(message.data);
      } else if (message.type === 'error') {
        console.error('WebView extraction error:', message.error);
        onSourcesExtracted?.(null);
      }
    } catch (parseError) {
  ...
  return (
    <View>
      <WebView
        ref={webviewRef}
        source={{ uri: 'https://megacloud.tv/about' }}
...
export default WebViewResolver;
