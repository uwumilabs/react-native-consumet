    ...
    try {
      ...
      const info: IMangaInfo = {
        ...
        status: this.determineMediaState(
          $(topInfoWrapper)
            .find(
              'div > div.flex.flex-row > div:nth-child(1) > h3:nth-child(2)'
            )
            .text()
            .trim()
        ),
        ...
        updatedOn: dom
          .find(
            '.grid.grid-cols-1.gap-5.mt-8 > div:nth-child(5) > h3:nth-child(2)'
          )
          .text()
          .trim(),
        genres: dom
        ...
        recommendations: dom
          ...
          .map((index, ele): IMangaResult => {
            return {
              ...
              status: this.determineMediaState(
                $(ele).find('div > div:nth-child(1) > span').text().trim()
              ),
              rating: $(ele)
                .find('div > div.block > span > label')
                .text()
                .trim(),
            };
          })
      ...
      const results = dom
        ...
        .map((index, ele): IMangaResult => {
          return {
            id: $(ele).attr('href') as string,
            title: $(ele)
              .find('div > div > div:nth-child(2) > span:nth-child(1)')
              .text()
              .trim() as string,
            image: $(ele)
              .find('div > div > div:nth-child(1) > img')
              .attr('src') as string,
            status: this.determineMediaState(
              $(ele).find('div > div > div:nth-child(1) > span').text().trim()
            ),
            latestChapter: $(ele)
              .find('div > div > div:nth-child(2) > span:nth-child(2)')
              .text()
              .trim(),
            rating: $(ele)
              .find('div > div > div:nth-child(2) > span:nth-child(3) > label')
              .text()
              .trim(),
          };
        })
