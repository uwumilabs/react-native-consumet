...
class Ummangurau extends MovieParser {
  ...
  protected override logo =
    'https://www1.ummagurau.com/images/group_1/theme_8/logo.png?v=0.1';
    ...
    try {
      ...
      $('div.flw-item').each((i, e) => {
        searchResult.results.push({
          ...
          type:
            $(e).find('span.fdi-type').text() === 'Movie'
              ? TvType.MOVIE
              : TvType.TVSERIES,
        });
      });
  ...
  override fetchMediaInfo = async (mediaId: string): Promise<IMovieInfo> => {
    ...
    try {
      ...
      movieInfo.type =
        $("a[title='TV Shows']").text() === '' ? TvType.TVSERIES : TvType.MOVIE;
      movieInfo.releaseDate = $('div.row-line')
        .text()
        .replace('Released: ', '')
        .trim();
