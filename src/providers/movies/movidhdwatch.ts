...
class MovieHdWatch extends MovieParser {
    ...
    try {
      ...
      $('.film_list-wrap > div.flw-item').each((i, el) => {
        const releaseDate = $(el)
          .find('div.film-detail > div.film-infor > span:nth-child(2)')
          .text();
        const duration = $(el)
          .find('div.film-detail > div.film-infor > span:nth-child(4)')
          .text();
  ...
  override fetchMediaInfo = async (mediaId: string): Promise<IMovieInfo> => {
    ...
    try {
      ...
      movieInfo.cover = $('div.dp-w-cover')
        .attr('style')
        ?.slice(22)
        .replace(')', '')
        .replace(';', '');
      ...
      movieInfo.rating = parseFloat(
        $('div.dp-i-stats > span.item.mr-2').text().replace('IMDB: ', '')
      );
      ...
      const movies = $('.section-id-02')
        ...
        .map((i, el) => {
          ...
          const movie: IMovieResult = {
            id: $(el).find('div.film-poster > a').attr('href')?.slice(1) ?? '',
            title:
              $(el).find('div.film-detail > h3.film-name > a').attr('title') ??
              '',
            url: `${this.baseUrl}${$(el).find('div.film-poster > a').attr('href')}`,
            image: $(el).find('div.film-poster > img').attr('data-src'),
            releaseDate: isNaN(parseInt(releaseDate)) ? undefined : releaseDate,
