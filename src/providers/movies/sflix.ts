...
class SFlix extends MovieParser {
    ...
    try {
      ...
      $('.film_list-wrap > div.flw-item').each((i, el) => {
        ...
        searchResult.results.push({
          ...
          type:
            $(el)
              .find('div.film-detail > div.fd-infor > span:nth-child(2)')
              .text() === 'Movie'
              ? TvType.MOVIE
              : TvType.TVSERIES,
        });
  ...
  override fetchMediaInfo = async (mediaId: string): Promise<IMovieInfo> => {
    ...
    try {
      ...
      movieInfo.image = $(
        '.dp-i-c-poster > div:nth-child(1) > img:nth-child(1)'
      ).attr('src');
      ...
      movieInfo.production = $('div.row > div.col-xl-6 .row-line')
        ...
        .get()
        .join(', ');
      movieInfo.country = $('div.row > div.col-xl-6 .row-line')
        .eq(1)
        .find('a')
        .text()
        .trim();
      ...
      movieInfo.rating = parseFloat(
        $('div.film-stats > div.fs-item > span.imdb')
          .text()
          .replace('IMDB: ', '')
      );
      ...
      switch (server) {
        case StreamingServers.Voe:
          return {
            ...
            ...(await new Voe(this.proxyConfig, this.adapter).extract(
              serverUrl
            )),
          };
      ...
      $(
        ...
        .each((i, el) => {
          const resultItem: IMovieResult = {
            id: $(el).find('div.film-poster > a').attr('href')?.slice(1) ?? '',
            title:
              $(el).find('div.film-detail > h2.film-name > a').attr('title') ??
              '',
            url: `${this.baseUrl}${$(el).find('div.film-poster > a').attr('href')}`,
            image: $(el).find('div.film-poster > img').attr('data-src'),
            type:
    ...
    try {
      ...
      $('div.swiper-slide').each((i, el) => {
        results.results.push({
          ...
          type:
            $(el).find('a').attr('href')?.split('/')[1] === 'movie'
              ? TvType.MOVIE
              : TvType.TVSERIES,
        });
      });
