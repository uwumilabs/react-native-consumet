...
export function createZoro(ctx: ProviderContext): AnimeParser {
  ...
  class ZoroImpl extends AnimeParser {
     ...
     */
    search(query: string, page: number = 1): Promise<ISearch<IAnimeResult>> {
      if (page <= 0) {
        page = 1;
      }
    ...
    public retrieveServerId = ($: any, index: number, subOrDub: SubOrSub) => {
     ...
     * @param url string
     */
    public scrapeCardPage = async (url: string, headers?: object): Promise<ISearch<IAnimeResult>> => {
      try {
        const res: ISearch<IAnimeResult> = {
          currentPage: 0,
          hasNextPage: false,
     ...
     */
    public scrapeCard = async ($: CheerioAPI): Promise<IAnimeResult[]> => {
      try {
