...
class Zoro extends AnimeParser {
  ...
  fetchAdvancedSearch(
    ...
    if (genres?.length) {
      const genreIds = genres
        .map((genre) => (mappings.genre![genre] || genre).toString())
        .join('%2C');
  ...
  fetchStudio(
    ...
    return this.scrapeCardPage(
      `${this.baseUrl}/producer/${studio}?page=${page}`
    );
  }
  /**
   * @param page number
    ...
    try {
      ...
      $('#slider div.swiper-wrapper div.swiper-slide').each((i, el) => {
        ...
        res.results.push({
          ...
          rank: parseInt(
            card.find('.desi-sub-text').text().match(/(\d+)/g)?.[0]!
          ),
          url: `${this.baseUrl}/${id}`,
          type: card
          ...
          sub:
            parseInt(card.find('div.sc-detail div.tick-sub').text().trim()) ||
            0,
          dub:
            parseInt(card.find('div.sc-detail div.tick-dub').text().trim()) ||
            0,
          episodes:
            parseInt(card.find('div.sc-detail div.tick-eps').text()) || 0,
  ...
  async fetchSearchSuggestions(query: string): Promise<ISearch<IAnimeResult>> {
    try {
      ...
      const { data } = await this.client.get(
        `${this.baseUrl}/ajax/search/suggest?keyword=${encodedQuery}`
      );
  ...
  async fetchContinueWatching(connectSid: string): Promise<IAnimeEpisode[]> {
    try {
      ...
      const { data } = await this.client.get(
        `${this.baseUrl}/user/continue-watching`,
        {
          headers: {
      ...
      $('.flw-item').each((i, ele) => {
        ...
        res.push({
          ...
          title: atag.text(),
          number: parseInt(
            card.find('.fdb-type').text().replace('EP', '').trim()
          ),
          duration: duration,
    ...
    try {
      ...
      $$('div.detail-infor-content > div > a').each((i, el) => {
        ...
        const isSubbed =
          number <=
          (parseInt(
            $('div.film-stats div.tick div.tick-item.tick-sub').text().trim()
          ) || 0);
        const isDubbed =
          number <=
          (parseInt(
            $('div.film-stats div.tick div.tick-item.tick-dub').text().trim()
          ) || 0);
    ...
    try {
      ...
      $('.flw-item').each((i, ele) => {
        ...
        const watchList = card
          .find('.dropdown-menu .added')
          .text()
          .trim() as WatchListType;
        const type = card
          .find('.fdi-item')
          ?.first()
          ?.text()
