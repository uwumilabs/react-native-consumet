...
class NineAnime extends AnimeParser {
  ...
  override async fetchAnimeInfo(animeUrl: string): Promise<IAnimeInfo> {
    ...
    try {
      ...
      animeInfo.otherNames = $('.names')
        ...
        .map((name) => name?.trim());
      animeInfo.hasSub =
        $('div#w-info > .binfo > .info > .meta .sub').length === 1;
      animeInfo.hasDub =
        $('div#w-info > .binfo > .info > .meta .dub').length === 1;
      ...
      const {
        ...
        `${this.baseUrl}/ajax/episode/list/${id}?vrf=${encodeURIComponent(vrf)}`
      );
      const $$ = load(result);
  ...
  override async fetchEpisodeSources(
    ...
    try {
      ...
      const serverSource = (
        await this.client.get(
          `${this.baseUrl}/ajax/server/${s.url}?vrf=${encodeURIComponent(serverVrf)}`
        )
      ).data;
      const embedURL = (
        await this.client.get(
      ...
      if (embedURL.startsWith('http')) {
        const response: ISource = await this.fetchEpisodeSources(
          embedURL,
          server
        );
