/**
 * Ext-movies — registry-powered movie/TV browser.
 *
 * Provider is fetched dynamically from the extension registry (GitHub CDN)
 * then wrapped with META.TMDB for normalised metadata + IDs.
 */
...
export default function ExtMoviesScreen() {
  // Registry / provider
  ...
  useEffect(() => {
    const exts: any[] = manager.getExtensionsByCategory('movies');
    setExtensions(exts);
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={S.infoRow}>
              ...
              <View style={S.infoMeta}>
                ...
                {detailItem?.rating ? (
                  <Text style={S.ratingLine}>★ {String(detailItem.rating).slice(0, 3)}</Text>
                ) : null}
              </View>
            </View>
            ...
            {seasonEps.length > 0 ? (
              <View style={S.section}>
                <Text style={S.sectionTitle}>{seasonEps.length === 1 ? 'Play' : 'Episodes'}</Text>
                <View style={S.epGrid}>
      ...
      <View style={S.playerRoot}>
        ...
        {loadingVideo ? (
          ...
          <Video
            ...
            resizeMode="contain"
            onError={(e) => {
              console.log('[ExtMovies] video error', JSON.stringify(e));
              setVideoErr(JSON.stringify((e as any)?.error ?? e));
            }}
            onLoad={(e) => console.log('[ExtMovies] video loaded', e)}
          />
        ) : null}
...
const S = StyleSheet.create({
  ...
  ratingLine: { fontSize: 13, color: colors.warn, fontWeight: '600' },
